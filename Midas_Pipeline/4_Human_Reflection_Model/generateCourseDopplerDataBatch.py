...
if __name__ == "__main__":
    ...
    for root, dirs, files in os.walk(args.input_folder):
        for d in dirs:
            ...
            # Check if the directory contains a .obj file
            if any(file.endswith(".obj") for file in os.listdir(dir_path)):
                #test(dir_path)

                input_path = os.path.join(root,d)
                relative_path = os.path.relpath(input_path,args.input_folder)
                output_path = os.path.join(args.output_folder,relative_path)
                input_paths.append(input_path)
                output_paths.append(output_path)
    numfolders = len(input_paths)
    ...
    with alive_bar(numfolders) as bar:
        for in_folder,out_folder in zip(input_paths,output_paths):
            ...
            #print(in_folder)
            doppler_data = np.array(doppler_time_data).T
            output_filename = os.path.join(out_folder, "synthDopplerData.npy")
            print(output_filename)
            np.save(output_filename, doppler_data)
            bar()
